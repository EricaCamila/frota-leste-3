...
function MapCenterAdjuster({ vehicles }) {
  const map = useMap()

  useEffect(() => {
    if (vehicles.length > 0) {
      // Calcular o centro baseado em todos os veículos
...
export default function LocationMap({ vehicles }: LocationMapProps) {
  useEffect(() => {
    fixLeafletIcon()
  }, [])
  ...
  return (
    <MapContainer center={defaultCenter} zoom={13} style={{ height: "100%", width: "100%" }}>
      ...
      <MapCenterAdjuster vehicles={vehicles} />

      {vehicles.map((vehicle) => (
        <div key={vehicle.id}>
          <Marker
            position={[vehicle.latitude, vehicle.longitude]}
            icon={vehicle.is_moving ? movingVehicleIcon : stoppedVehicleIcon}
          >
